<template>
	<div class="add_airportname">
		<Loader v-if="laoding"/>
		<div class="card">
		  	<div class="card-header">
			    <h4 class="title_user">
			    	<i class="sidebar_icon fas fa-plane"></i>
				    Добавить название аэропорта
				</h4>
...
<script>
	...
	export default{
		...
		computed:{
			...mapGetters('airportname',['getMassage'])
		},
		mounted(){
			this.laoding = false
		},
		methods:{
			...mapActions('airportname',['actionAddAirportname']),
			isRequired(input){
			...
			async saveAirportname(){
		    	if (this.form.name != ''){
					this.laoding = true
					await this.actionAddAirportname(this.form)
