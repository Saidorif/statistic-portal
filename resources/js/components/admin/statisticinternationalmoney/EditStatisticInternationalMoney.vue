		...
		<div class="card">
		  	<div class="card-header">
			    <h4 class="title_user">
					...
					Добавить Международное денежное обращение
				</h4>
				<router-link class="btn_black" to="/crm/statisticinternationalmoney">
					...
					<div class="row">
					  <div class="form-group col-md-4">
					    ...
					    <select name="" v-model="form.type" :class="isRequired(form.type) ? 'isRequired' : ''" class="form-control input_style">
					    	<option value="export" selected disabled>Экспорт</option>
					    </select>
					  </div>
					  <div class="form-group col-md-4">
...
<script>
	...
	export default{
		...
		computed:{
			...mapGetters('statisticinternationalmoney',['getMassage','getStatisticInternationalMoney']),
			...mapGetters('region',['getRegionList']),
		},
		async mounted(){
			await this.actionEditStatisticInternationalMoney(this.$route.params.statisticinternationalmoneyId)
			await this.actionRegionList()
			this.form = this.getStatisticInternationalMoney
			this.laoding = false
		},
		methods:{
			...mapActions('statisticinternationalmoney',['actionUpdateStatisticInternationalMoney','actionEditStatisticInternationalMoney']),
			...
			async saveStatistic(){
		    	if (this.form.year != ''){
					this.laoding = true
					await this.actionUpdateStatisticInternationalMoney(this.form)
					if (this.getMassage.success){
						...
						this.$router.push("/crm/statisticinternationalmoney");
