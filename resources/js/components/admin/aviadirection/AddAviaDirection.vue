<template>
	<div class="add_aviadirection">
		<Loader v-if="laoding"/>
		<div class="card">
		  	<div class="card-header">
			    <h4 class="title_user">
			    	<i  class="peIcon fas-fa plane"></i>
				    Добавить Авиайўналишлари
				</h4>
...
<script>
	...
	export default{
		...
		computed:{
			...mapGetters('aviadirection',['getMassage'])
		},
		mounted(){
			this.laoding = false
		},
		methods:{
			...mapActions('aviadirection',['actionAddAviaDirection']),
			isRequired(input){
			...
			async saveAviaDirection(){
		    	if (this.form.name != ''){
					this.laoding = true
					await this.actionAddAviaDirection(this.form)
