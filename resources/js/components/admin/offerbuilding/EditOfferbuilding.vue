<template>
	<div class="edit-offerbuilding">
		<Loader v-if="laoding"/>
		<div class="card">
		  	<div class="card-header">
			    <h4 class="title_user">
			    	<i class="peIcon fas fa-building"></i>
				    Изменить Offer building 
				</h4>
				<router-link class="btn_black" to="/crm/offerbuilding"><span class="peIcon fas fa-arrow-left"></span> Назад</router-link>
		  	</div>
		  	<div class="card-body">
		  		<form @submit.prevent.enter="saveOfferbuilding" >
					<div class="row">
					  <div class="form-group col-md-9">
					    <label for="name">Область</label>
					    <input
					    	type="text"
					    	class="form-control input_style"
					    	id="name"
					    	placeholder="Область"
					    	v-model="form.name"
					    	:class="isRequired(form.name) ? 'isRequired' : ''"
				    	>
					  </div>
					  <div class="form-group col-lg-3 form_btn">
					  	<button type="submit" class="btn btn-primary btn_save_category">
					  		<i class="fas fa-save"></i>
						  	Сохранить
						</button>
				  	  </div>
					</div>
				</form>
		  	</div>
	  	</div>
	</div>
</template>
<script>
	import { mapGetters , mapActions } from 'vuex'
	import Loader from '../../Loader'
	export default{
		components:{
			Loader
		},
		data(){
			return{
				form:{
					id:'',
					name:'',
				},
				requiredInput:false,
				laoding: true
			}
		},
		computed:{
			...mapGetters('offerbuilding',['getMassage'])
		},
		async mounted(){
			await this.actionEditOfferbuilding(this.$route.params.offerbuildingId)
			this.laoding = false
		},
		methods:{
			...mapActions('offerbuilding',['actionEditOfferbuilding','actionUpdateOfferbuilding']),
			isRequired(input){
	    		return this.requiredInput && input === '';
		    },
			async saveOfferbuilding(){
		    	if (this.form.name != ''){
					this.laoding = true
					await this.actionUpdateOfferbuilding(this.form)
					if (this.getMassage.success) {
						toast.fire({
					    	type: 'success',
					    	icon: 'success',
							title: this.getMassage.message,
					    })
						this.$router.push("/crm/offerbuilding");
						this.requiredInput = false
					}
					this.laoding = false
				}else{
					this.requiredInput = true
				}
		    }
		}
	}
</script>
<style scoped>

</style>
